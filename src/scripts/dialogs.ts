...
export function useChargeDialog(el: Element) {
...
export function useCloseDialog(el: Element) {
  el.addEventListener('click', e => {
    e.preventDefault()
...
export function useDialogs(cb: (elements: Element[]) => void) {
...
export function initMaskedTextFields(selector: string, mask: AnyMaskedOptions): void {
  ...
  elements.forEach(el => IMask(el as HTMLElement, mask))
}
